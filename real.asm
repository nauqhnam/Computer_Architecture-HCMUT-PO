...
# Write board to file with formatting similar to display
writeBoardToFile:
    # Save return address
    addi $sp, $sp, -4
    sw $ra, 0($sp)
    ...
    # Write custom horizontal line that's guaranteed to be consistent
    move $a0, $s7          # File descriptor
    
    # Create a consistent horizontal line in buffer
    la $t0, lineBuffer
    li $t1, 0              # Counter
    li $t2, 65             # Line length (matches display width)
...
createHorLine:
    ...
    # Write the custom horizontal line
    la $a1, lineBuffer     # Buffer address
    move $a2, $t1          # Buffer length (line + newline)
    li $v0, 15             # Write syscall
    syscall
    
    # Write column numbers
    move $a0, $s7          # File descriptor
    la $a1, colNumbers     # Column numbers
    
    # Calculate length of column numbers
    move $t0, $a1
    li $t1, 0
colNumLoop:
    lb $t2, ($t0)
    beqz $t2, endColNumLoop
    addi $t0, $t0, 1
    addi $t1, $t1, 1
    j colNumLoop
endColNumLoop:
    move $a2, $t1          # Line length
...
writeRowLoop:
    ...
    beq $t1, 8, writeRow8
    beq $t1, 9, writeRow9
    beq $t1, 10, writeRow10
    beq $t1, 11, writeRow11
    beq $t1, 12, writeRow12
    beq $t1, 13, writeRow13
    beq $t1, 14, writeRow14
    
continueWriteRow:
    # Get the row format string and write it
    move $a0, $s7          # File descriptor
    move $a1, $t9          # Row format string
...
rowFmtLoop:
    lb $t2, ($t0)
    beqz $t2, endRowFmtLoop
    addi $t0, $t0, 1
    addi $t8, $t8, 1
    j rowFmtLoop
endRowFmtLoop:
    move $a2, $t8          # Format string length
    
    li $v0, 15             # Write syscall
    syscall
    
    # Now write the cells for this row
    li $t2, 0              # Column counter
...
writeColLoop:
    ...
    # Get and write cell content
    lb $t5, ($t4)          # Cell content
    
    # Create a small buffer for cell with consistent spacing
    la $t6, lineBuffer
    sb $t5, 0($t6)         # Cell content
    li $t5, ' '
    sb $t5, 1($t6)         # Space
    li $t5, ' '
    sb $t5, 2($t6)         # Space
    ...
    move $a0, $s7          # File descriptor
    move $a1, $t6          # Cell buffer
    li $a2, 3              # Length (cell + 2 spaces)
    li $v0, 15             # Write syscall
    syscall
    ...
    # Check if all rows displayed
    li $t3, 15
    bne $t1, $t3, writeRowLoop
    
    # Write closing horizontal line with same length as opening line
    move $a0, $s7          # File descriptor
    
    # Create a consistent horizontal line in buffer (same as opening line)
    la $t0, lineBuffer
    li $t1, 0              # Counter
    li $t2, 65             # Line length (matches display width)
...
# Write result message to file with better formatting
writeResultToFile:
    # a0 = result message
    ...
    # Write an empty line before the result message
    move $a0, $s7
    la $t6, lineBuffer
    li $t5, '\n'
    sb $t5, 0($t6)
